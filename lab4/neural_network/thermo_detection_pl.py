...
net_rgb = None
if FUSION == "EARLY":
    net_fus = cv2.dnn.readNet('configuration/yolov3_training_last_f.weights', 'configuration/yolov3_testing_f.cfg')
if FUSION == "LATE":
    net_therm = cv2.dnn.readNet('configuration/yolov3_training_last_t.weights', 'configuration/yolov3_testing_t.cfg')
    net_rgb = cv2.dnn.readNet('configuration/yolov3_training_last_c.weights', 'configuration/yolov3_testing_c.cfg')
...
for i in range(200, 300): # można zmieniać range do 518
    ...
    if FUSION == "LATE":
        ...
        for rect1 in Rect1:
            for rect2 in Rect2:
                if IoU(rect1, rect2) > 0:
                    boxes_iou.append([(Rect1.index(rect1), Rect2.index(rect2)), IoU(rect1, rect2)])
        boxes_iou = sorted(boxes_iou, key=lambda a: a[1], reverse=True)

        Rect1_paired, Rect2_paired, paired_boxes = [], [], []
        for box_iou in boxes_iou:
            indexes = box_iou[0]
            iou = box_iou[1]
            if indexes[0] not in Rect1_paired and indexes[1] not in Rect2_paired:
                paired_boxes.append(indexes)
                Rect1_paired.append(indexes[0])
                Rect2_paired.append(indexes[1])
        for paired_box in paired_boxes:
            r1 = Rect1[paired_box[0]]
            r2 = Rect2[paired_box[1]]
